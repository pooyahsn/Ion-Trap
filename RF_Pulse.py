...
args={'omegas':omegas,'omegad':omegad,'angfrequency':omegad}
...
Rho=result.states
rho=[qp.ptrace(state,1) for state in Rho]
shownum=100
fig, ax = plt.subplots()
ax.plot(result.times, [qp.expect(ud6u,state) for state in rho])
ax.plot(result.times, [qp.expect(ud5u,state) for state in rho])
ax.plot(result.times, [qp.expect(ud4u,state) for state in rho])
ax.plot(result.times, [qp.expect(ud3u,state) for state in rho])
ax.plot(result.times, [qp.expect(ud2u,state) for state in rho])
ax.plot(result.times, [qp.expect(ud0u,state) for state in rho])
...
plt.show()
fig, ax = plt.subplots()
ax.plot(result.times, [qp.expect(ee,state) for state in rho])
ax.plot(result.times, [qp.expect(gg,state) for state in rho])
